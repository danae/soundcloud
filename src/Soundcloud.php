...
class Soundcloud
{
  ...
  private function request(string $method, string $uri, array $query = [], array $jsonBody = [])
  {
    try
    {
      ...
      if (!preg_match('/[2-3][0-9]{2}/', $response->getStatusCode()))
        throw new SoundcloudException("The request \"{$method} {$uri}\" returned with HTTP status code {$response->getStatusCode()}: {$response->getBody()}");
  ...
  {
    return $this->get('/oembed',array_merge([
      'url' => $url,
      'format' => 'json'
    ],$query));
  }
